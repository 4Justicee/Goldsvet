...
$action="spin";
	...
	if(!isset($_SESSION['4ofking_fsc'])){
	$_SESSION['4ofking_fsc']=0;	
	}
	
	if($_SESSION['4ofking_fsc']<=0){
	 cange_balance($userId, $allbet*-1);
	 change_bank($user_id,'4ofking',$allbet,"spin");
	}
	////////////////////////////////
	...
	$linesId[5]=array(1,2,2,2,1);
	$linesId[6]=array(1,0,0,0,1);
	
	$linesId[7]=array(2,2,1,0,0);
	$linesId[8]=array(0,0,1,2,2);
	
	$linesId[9]=array(2,1,1,1,0);
    ...
    $reels[0]=array(1,2,3,4,5,6,7);
	$reels[1]=array(1,2,3,4,5,6,7);
	$reels[2]=array(1,2,3,4,5,6,7);
	$reels[3]=array(1,2,3,4,5,6,7);
	$reels[4]=array(1,2,3,4,5,6,7);
...
$isWin=false;
	...
	array_push($reels2,$reels[0]);
	array_push($reels2,$reels[1]);
	array_push($reels2,$reels[2]);
	array_push($reels2,$reels[3]);
	array_push($reels2,$reels[4]);
...
$reels2[$rndRow][$rndCell]=8;	
$reels2[$rndRow][$rndCell-1]=8;	
$reels2[$rndRow][$rndCell+1]=8;	
	
$reels2[$rndRow-1][$rndCell]=8;	
$reels2[$rndRow-1][$rndCell-1]=8;	
$reels2[$rndRow-1][$rndCell+1]=8;		
...
$tmpWinStr='{"@type":"logic.api.paytable.slot.spinresult.Spin_Result_Item","positions":[{"@type":"logic.api.paytable.slot.spinresult.Position2D","xPos":'.$rndRow.',"yPos":'.$rndCell.'}],"icons":[8],"jokers":[0],"strip_icons":[8],"animation_id":null,"layer_id":null,"fullscreen_animation_id":null,"combo_id":42,"line_number":0,"win":0,"playing_symbol":-1,"win_multiplier":1,"bonus_id":"EXPANDING_JOKER","feature_state_index":0,"win_data":null,"fullscreenAnimation_id":null}';
...
$isBonus=false;	
 ...
 change_bank($user_id,'4ofking',($totalWin)*-1,"spin");	
...
function GetDouble(){
	...
	$_SESSION['4ofking_dc']=$dealerCard;
	$_SESSION['4ofking_dcstr']='{"@type":"logic.api.gamelogic.gamble.Card","rank":'.$dealerCard.',"is_joker":false,"suit":"'.$cardSuits[0].'"}';
	shuffle($cardSuits);
	
	$userId=$user_id;
    $user_balance =sprintf("%01.2f",get_balance($userId));
	$_SESSION['4ofking_dcnt']=10;
	$rtn='{"@type":"MethodInvocation","payload":{"@type":"logic.api.gamelogic.Feature_State","feature_id":"GAMBLE","id":2,"init_parameters":{"@type":"logic.api.gamelogic.gamble.Gamble_Init_Parameters","start_win":'.$_SESSION['4ofking_win'].',"mults":[2],"tries":10},"current_state":{"@type":"logic.api.gamelogic.gamble.BTD_Gamble_Current_State","strips_current":null,"strips_next":null,"strips_changed_bet":null,"curr_win":'.$_SESSION['4ofking_win'].',"tries":10,"gamble_reactions":[],"dealer_card":{"@type":"logic.api.gamelogic.gamble.Card","rank":'.$dealerCard.',"is_joker":false,"suit":"'.$cardSuits[0].'"},"must_play":false},"prev_current_state":{"@type":"logic.api.gamelogic.gamble.BTD_Gamble_Current_State","strips_current":null,"strips_next":null,"strips_changed_bet":null,"curr_win":60,"tries":10,"gamble_reactions":[],"dealer_card":{"@type":"logic.api.gamelogic.gamble.Card","rank":12,"is_joker":false,"suit":"HEARTS"},"must_play":false},"current_win":0,"status":"IN_PROGRESS","last_step":null,"parent_index":0},"requestId":"1","method":"START_FEATURE"}';
	return $rtn;	
	}
...
function StepDouble(){
	...
	$userId=$user_id;
	cange_balance($userId,$_SESSION['4ofking_win']*-1);
    change_bank($user_id,'4ofking',$_SESSION['4ofking_win'],"double");	
	
	$dwin=$_SESSION['4ofking_win'];
	$dwins=$dwin;
	$rnd_double=rand(1,2);
	$isWin="false";
	if($rnd_double==1){
	$uCard=rand($_SESSION['4ofking_dc'],12);
    $dwin=$dwin*2;	
	$isWin="true";
	}else{
	$dwin=0;	
    $uCard=rand(0,$_SESSION['4ofking_dc']);		
	}
	
	/*if($uCard==$_SESSION['4ofking_dc']){
	$dwin=$_SESSION['4ofking_win'];	
	$isWin="true";
	$dwins=$dwin;
	}*/
    ...
    change_bank($user_id,'4ofking',$dwin*-1,"double");	
