	...
	var p =
	{
		init: function()
		{
			...
			this._view = new WinVisualizationView(new WinVisualizationViewConfig(spinMessage.reels, spinMessage.lines, spinMessage.scatters, settings.wildIndex, null,
											...
											linesViewConfig, settings.lineGame, settings.comboColors, settings.multiplierImages));
		...
		start: function()
		{
			...
			{
				var snd = null;
				if (spinMessage.freeSpins && spinMessage.freeSpinScatters )
				{
					...
					for (var i = 0; i < len; i++)
					{
						...
						{
							if (settings.freespinGame && scatterConfig.freespinsSoundIndexInFs != undefined)
							{
								snd = settings.freespinSounds[scatterConfig.freespinsSoundIndexInFs];
							} else {
