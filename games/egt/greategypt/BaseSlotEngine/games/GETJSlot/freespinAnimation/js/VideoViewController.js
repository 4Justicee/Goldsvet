	...
	function createVideo(col, row, expand, startFromLoop)
	{
		...
		if (spinMessage.freeSpinScatters && scatterConfig && scatterConfig.freespinsVideoIndex != -1)
		{
			...
			for (var i = 0; i < len; i++)
			{
				...
				{
					if (settings.freespinGame && scatterConfig.freespinsVideoIndexInFS != undefined)
					{
						card = scatterConfig.freespinsVideoIndexInFS;
					}
					else
					{
						card = scatterConfig.freespinsVideoIndex;
					}
					//videoConfig = settings.reelVideos[card];
					break;
				}
			}
		}
