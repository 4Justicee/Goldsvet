...
$res1=mysql_fetch_assoc(mysql_query("SELECT g_id FROM game_settings WHERE g_name='greategypt'"));
...
$_SESSION['greategypt_fsc2']=0;
	...
	$_SESSION['greategypt_win']=$hisInfo['winAmount'];	
	}
	
	$userId=$user_id;
    $user_balance =get_balance($userId)*100 ;
	$user_balance-=$_SESSION['greategypt_win'];
	
	return '{"sessionKey": "'.$_GET['sessionKey'].'", "eventTimestamp":'.time().', "qName": "app.services.messages.response.LoginResponse", "currency": "EUR", "multigame": true, "groups": ["all", "myGames", "fruit", "classic", "multiline", "extraline", "diceSlots", "keno", "roulette", "cards"], "showRtp": false, "autoplayLimit": 0, "playerName": "egt01", "languages": ["ro", "nl", "it", "bg", "en", "es", "ru", "fr", "go"], "complex": {"GETJSlot": [{"mlmJackpot": true, "recovery": "norecovery", "gameName": "Dragon Reels", "featured": false, "groups": [{"name": "all", "order": 16}, {"name": "fruit", "order": 5}, {"name": "classic", "order": 31}, {"name": "multiline", "order": 35}, {"name": "extraline", "order": 22}, {"name": "diceSlots", "order": 29}, {"name": "keno", "order": 16}, {"name": "roulette", "order": 16}, {"name": "cards", "order": 19}, {"name": "myGames", "order": 17}], "totalBet": 0, "gameIdentificationNumber": 1}]}, "command": "login", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.'}';	
...
//        $contractDateEnd = date('Y-m-d', strtotime("01/01/2019"));
            ...
            $ptb[0]=array(0,0,10,20,50,100);  //9
            $ptb[1]=array(0,0,0,10,20,100);   //10
            $ptb[2]=array(0,0,0,10,20,100);     //11
            $ptb[3]=array(0,0,0,10,20,100);     //12

            $ptb[4]=array(0,0,0,10,40,200); // k
            $ptb[5]=array(0,0,0,10,40,200); // A

            $ptb[6]=array(0,0,0,10,100,400); // dog
...
$isCalc=false;
 ...
 change_bank($user_id,'greategypt',($totalWin/100)*-1,"spin");	
	...
	$gameState="freespin";
	if($_SESSION['greategypt_fsc']<=0){
	$_SESSION['greategypt_fsc']=16;
	$_SESSION['greategypt_fsc2']=0;
	$fsc=15;
	}else{
	$_SESSION['greategypt_fsc']+=15;	
	$fsc=15;
	}
	}
...
$gameState="idle";
  $user_balance =get_balance($userId)*100 ;
	...
	$_SESSION['greategypt_j1']=0;
	$_SESSION['greategypt_j2']=0;
	$_SESSION['greategypt_j3']=0;
	$_SESSION['greategypt_j4']=0;
	...
	$logstr.='winAmount='.$_SESSION['greategypt_win'].'##';
	$logstr.='gameState='.$gameState.'##';
	$logstr.='ws='.$ws.'##';
	$logstr.='freespins='.$_SESSION['greategypt_fsc'].'##';
	$logstr.='freespinsUsed='.$_SESSION['greategypt_fsc2'].'##';
	$logstr.='scatters='.$scatterCells.'##';
	$logstr.='exp='.$_SESSION['greategypt_exp'].'##';
	$logstr.='freespinScatters='.$scatterCells2.'##';
	
	$_SESSION['greategypt_reelString']=$reelString;
	/*history*/
...
$sets=get_game_settings('greategypt');
...
set_game_settings ('greategypt','g_bon1_1',$jacks_str);
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4,$logstr;	
	...
	$_SESSION['greategypt_j'.$jType]++;
	
	if($_SESSION['greategypt_j'.$jType]>=3){
	$jackState="idle";	
	$sets=get_game_settings('greategypt');
...
$jacks_str=implode("|",$jacks);
	...
	$_SESSION['greategypt_win']=$winAmount;
	...
	/*history*/
	$logstr.='reelString='.$_SESSION['greategypt_reelString'].'##';
	$logstr.='winAmount='.$_SESSION['greategypt_win'].'##';
	if($winAmount>0){
...
function InitGame2(){
	...
	unset($_SESSION['greategypt_j1']);
	unset($_SESSION['greategypt_j2']);
	unset($_SESSION['greategypt_j3']);
	unset($_SESSION['greategypt_j4']);
	$res2=mysql_fetch_assoc(mysql_query("SELECT str FROM game_log WHERE game_id='".$res1['g_id']."' AND user_id='$user_id' ORDER BY id DESC LIMIT 1"));
	...
	$_SESSION['greategypt_reelString']=$hisInfo['reelString'];
	...
	$_SESSION['greategypt_fsc']=$hisInfo['freespins'];
	$_SESSION['greategypt_fsc2']=$hisInfo['freespinsUsed'];
	}
	if(isset($hisInfo['freespins'])){
	$_SESSION['greategypt_exp']=$hisInfo['freespins'];
	}
