...
$res1=mysql_fetch_assoc(mysql_query("SELECT g_id FROM game_settings WHERE g_name='dragon_reels'"));
...
$_SESSION['dragon_reels_fsc']=0;
$_SESSION['dragon_reels_fsc2']=0;
	...
	$_SESSION['dragon_reels_reelString']=$hisInfo['reelString'];
	...
	$_SESSION['dragon_reels_win']=$hisInfo['winAmount'];	
	}
	
	$userId=$user_id;
    $user_balance =get_balance($userId)*100 ;
	$user_balance-=$_SESSION['dragon_reels_win'];
	
	return '{"sessionKey": "'.$_GET['sessionKey'].'", "eventTimestamp":'.time().', "qName": "app.services.messages.response.LoginResponse", "currency": "EUR", "multigame": true, "groups": ["all", "myGames", "fruit", "classic", "multiline", "extraline", "diceSlots", "keno", "roulette", "cards"], "showRtp": false, "autoplayLimit": 0, "playerName": "egt01", "languages": ["ro", "nl", "it", "bg", "en", "es", "ru", "fr", "go"], "complex": {"DRJSlot": [{"mlmJackpot": true, "recovery": "norecovery", "gameName": "Dragon Reels", "featured": false, "groups": [{"name": "all", "order": 16}, {"name": "fruit", "order": 5}, {"name": "classic", "order": 31}, {"name": "multiline", "order": 35}, {"name": "extraline", "order": 22}, {"name": "diceSlots", "order": 29}, {"name": "keno", "order": 16}, {"name": "roulette", "order": 16}, {"name": "cards", "order": 19}, {"name": "myGames", "order": 17}], "totalBet": 0, "gameIdentificationNumber": 1}]}, "command": "login", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.'}';	

	
	 //set_stat_game($userId, $user_balance, 0,0,"state_dragon_reels");
	return $rtn;	
	}
	...
	if(!isset($_SESSION['dragon_reels_fsc'])){
	$_SESSION['dragon_reels_fsc']=0;	
	}
	
	if($_SESSION['dragon_reels_fsc']<=0){
	 cange_balance($userId, $allbet*-1);
	 change_bank($user_id,'dragon_reels',$allbet,"spin");
	}
	////////////////////////////////
...
//	$ptb[12]=array(0,0,2,4,50,100);
	...
	$ptb[12]=array(0,0,50,100,1250,2500);
	/*-------------*/
...
$isBonus=false;	
 ...
 change_bank($user_id,'dragon_reels',($totalWin/100)*-1,"spin");	
	...
	$gameState="freespin";
	if($_SESSION['dragon_reels_fsc']<=0){
	$_SESSION['dragon_reels_fsc']=11;
	$_SESSION['dragon_reels_fsc2']=0;
	$fsc=10;
	}else{
	$_SESSION['dragon_reels_fsc']+=5;	
	$fsc=10;
	}
	}
	...
	if($_SESSION['dragon_reels_fsc']<=0){
	$_SESSION['dragon_reels_win']=$totalWin;
	}else{
	$_SESSION['dragon_reels_win']+=$totalWin;
$totalWin=$_SESSION['dragon_reels_win'];	
$gameState="idle";
  $user_balance =get_balance($userId)*100 ;
	}
	...
	$_SESSION['dragon_reels_j1']=0;
	$_SESSION['dragon_reels_j2']=0;
	$_SESSION['dragon_reels_j3']=0;
	$_SESSION['dragon_reels_j4']=0;
	...
	if($_SESSION['dragon_reels_fsc']>0){
	$_SESSION['dragon_reels_fsc']--;	
	$_SESSION['dragon_reels_fsc2']++;	
	
	}
	/*history*/
	$logstr.='reelString='.$reelString.'##';
	$logstr.='winAmount='.$_SESSION['dragon_reels_win'].'##';
	$logstr.='gameState='.$gameState.'##';
	$logstr.='ws='.$ws.'##';
	$logstr.='freespins='.$_SESSION['dragon_reels_fsc'].'##';
	$logstr.='freespinsUsed='.$_SESSION['dragon_reels_fsc2'].'##';
	$logstr.='scatters='.$scatterCells.'##';
	$logstr.='exp='.$_SESSION['dragon_reels_exp'].'##';
	$logstr.='freespinScatters='.$scatterCells2.'##';
	
	$_SESSION['dragon_reels_reelString']=$reelString;
	/*history*/
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4,$logstr;	
	...
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$_SESSION['dragon_reels_win'].', "eventTimestamp": '.time().', "gameNumber": 1561257, "state": "idle", "complex": {"gameCommand": "collect"}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';	

	
	
	/*history*/
	$logstr.='reelString='.$_SESSION['dragon_reels_reelString'].'##';
	$logstr.='winAmount=0##';
...
$sets=get_game_settings('dragon_reels');
...
set_game_settings ('dragon_reels','g_bon1_1',$jacks_str);
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4,$logstr;	
	...
	$_SESSION['dragon_reels_j'.$jType]++;
	
	if($_SESSION['dragon_reels_j'.$jType]>=3){
	$jackState="idle";	
	$sets=get_game_settings('dragon_reels');
...
$jacks_str=implode("|",$jacks);
	...
	$_SESSION['dragon_reels_win']=$winAmount;
...
function DoubleGame(){
	...
	$doubleWin=$_SESSION['dragon_reels_win'];
	
	 cange_balance($userId, ($doubleWin/100)*-1);
 change_bank($user_id,'dragon_reels',($doubleWin/100),"double");	
 ...
 change_bank($user_id,'dragon_reels',($doubleWin/100)*-1,"double");	

	}
	
	
	$_SESSION['dragon_reels_win']=$doubleWin;
	/*history*/
	$logstr.='reelString='.$_SESSION['dragon_reels_reelString'].'##';
	$logstr.='winAmount='.$_SESSION['dragon_reels_win'].'##';
	$logstr.='gameState='.$gameState.'##';
...
function InitGame2(){
	...
	unset($_SESSION['dragon_reels_j1']);
	unset($_SESSION['dragon_reels_j2']);
	unset($_SESSION['dragon_reels_j3']);
	unset($_SESSION['dragon_reels_j4']);
	$res2=mysql_fetch_assoc(mysql_query("SELECT str FROM game_log WHERE game_id='".$res1['g_id']."' AND user_id='$user_id' ORDER BY id DESC LIMIT 1"));
