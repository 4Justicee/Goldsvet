...
(function()
{
	var p =
	{
		init: function(config)
		{
			...
			this._super("startView", 85, 35);
			this.settings = GameSettings.getInstance();
			this.setSize(949, 567);
			...
			this._background = new CanvasView("", 85, 35);
			// this._background = new CanvasView("", 240, 133);
		...
		_onShowIntroTextAsStartWithBlinking: function()
		{
			...
			self._context.clearRect(0, 0, self._img.width, self._img.height);
		...
		dispose:function ()
		{
			...
			if(this.haveBlinkingText){
				console.log("StartAnimation dispose clearTimeout");
				this.haveBlinkingText = false;
	...
	function createAndPositionLinesForScreen(translations, valueText) {
		...
		this.blinkingInterval = this._defaultSettings.blinkingIntervalMiliSec;
		...
		if (typeof(valueText) !== 'undefined') {
			...
			if(this._config.currency)
			{
				setFont.call(this, valueFontSize/2, numberFontWeight, fontFace);
