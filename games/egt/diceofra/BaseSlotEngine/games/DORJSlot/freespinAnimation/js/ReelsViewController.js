		...
		for (var i = 0; i < settings.numReelCards; i++)
		{
			...
			if (this._scatterCount[card] != undefined)
			{
				...
				for (var j = 0; j < len; j++)
					...
					var freespinMinCount = settings.freespinGame && scatterConfig.freespinMinCountInFs != undefined ? 
					scatterConfig.freespinMinCountInFs : scatterConfig.freespinMinCount;
						
				if (freespinMinCount == -1)
					continue;
				...
				if (this._scatterCount[card] >= freespinMinCount - maxScatterOnReel)
				{
