...
$_SESSION['diceofra_fsc']=0;
$_SESSION['diceofra_jack']=0;
	...
	return '{"sessionKey": "'.$_GET['sessionKey'].'", "eventTimestamp":'.time().', "qName": "app.services.messages.response.LoginResponse", "currency": "EUR", "multigame": true, "groups": ["all", "myGames", "fruit", "classic", "multiline", "extraline", "diceSlots", "keno", "roulette", "cards"], "showRtp": false, "autoplayLimit": 0, "playerName": "egt01", "languages": ["ro", "nl", "it", "bg", "en", "es", "ru", "fr", "go"], "complex": {"DORJSlot": [{"mlmJackpot": true, "recovery": "norecovery", "gameName": "Dragon Reels", "featured": false, "groups": [{"name": "all", "order": 16}, {"name": "fruit", "order": 5}, {"name": "classic", "order": 31}, {"name": "multiline", "order": 35}, {"name": "extraline", "order": 22}, {"name": "diceSlots", "order": 29}, {"name": "keno", "order": 16}, {"name": "roulette", "order": 16}, {"name": "cards", "order": 19}, {"name": "myGames", "order": 17}], "totalBet": 0, "gameIdentificationNumber": 1}]}, "command": "login", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.'}';	

	
	 //set_stat_game($userId, $user_balance, 0,0,"state_diceofra");
	return $rtn;	
	}
	...
	if(!isset($_SESSION['diceofra_fsc'])){
	$_SESSION['diceofra_fsc']=0;	
	}
	
	if($_SESSION['diceofra_fsc']<=0){
	 cange_balance($userId, $allbet*-1);
	 change_bank($user_id,'diceofra',$allbet,"spin");
	}
	////////////////////////////////
...
//	$ptb[12]=array(0,0,2,4,50,100);
        ...
        $ptb[0]=array(0,0,10,10, 20, 100);
        $ptb[1]=array(0,0,0,10, 20, 100);
        $ptb[2]=array(0,0,0,10, 20, 100);
        $ptb[3]=array(0,0,0,10, 20, 100);
        $ptb[4]=array(0,0,0,10, 40, 200);

        $ptb[5]=array(0,0,0,10,40,200);
        $ptb[6]=array(0,0,0,10,100,400);
        $ptb[7]=array(0,0,0,10,100,400);
        $ptb[8]=array(0,0,0,10,100,400);

        $ptb[9]=array(0,0,10,100,1000,10000);
        $ptb[10]=array(0,0,10,100,1000,10000);

        $ptb[11]=array(0,0,100,1000,5000,10000);

        $ptb[12]=array(0,0,2,4,500,100);
...
$isBonus=false;	
 ...
 change_bank($user_id,'diceofra',($totalWin/100)*-1,"spin");	
	...
	$gameState="freespin";
	if($_SESSION['diceofra_fsc']<=0){
	$_SESSION['diceofra_fsc']=16;
	$fsc=15;
	}else{
	$_SESSION['diceofra_fsc']+=15;	
	$fsc=15;
	}
	}
	...
	if($_SESSION['diceofra_fsc']<=0){
	$_SESSION['diceofra_win']=$totalWin;
	}else{
	$_SESSION['diceofra_win']+=$totalWin;
$totalWin=$_SESSION['diceofra_win'];	
$gameState="idle";
  $user_balance =get_balance($userId)*100 ;
	}
	...
	$_SESSION['diceofra_j1']=0;
	$_SESSION['diceofra_j2']=0;
	$_SESSION['diceofra_j3']=0;
	$_SESSION['diceofra_j4']=0;
	
	}else{
	$isJackStr="false";	
	}
	
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$totalWin.', "eventTimestamp": '.time().', "gameNumber": '.time().', "state": "'.$gameState.'", "complex": {"jackpot": '.$isJackStr.', "freespins": '.$fsc.', "scatters": ['.$scatterCells.'], "expand": [], "gambles": 0, "gameCommand": "bet", "lines": ['.$ws.'], "reels": ['.$reelString.'], "freespinScatters": ['.$scatterCells2.']}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4;	
	...
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$_SESSION['diceofra_win'].', "eventTimestamp": '.time().', "gameNumber": '.time().', "state": "idle", "complex": {"gameCommand": "collect"}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';	
...
$sets=get_game_settings('diceofra');
...
set_game_settings ('diceofra','g_bon1_1',$jacks_str);
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4;	
	...
	$_SESSION['diceofra_j'.$jType]++;
	
	if($_SESSION['diceofra_j'.$jType]>=3){
	$jackState="idle";	
	$sets=get_game_settings('diceofra');
...
$jacks_str=implode("|",$jacks);
	...
	$_SESSION['diceofra_win']=$winAmount;
...
function PingGame(){
  ...
  $rtn2='{"complex": {"levelI": '.$jp1.', "levelII":'.$jp2.', "levelIII":'.$jp3.', "levelIV":'.$jp4.', "winsLevelI": 9990, "largestWinLevelI": 9990, "largestWinDateLevelI": "07-12-2016 01:19", "largestWinUserLevelI": "egt02", "lastWinLevelI": 1108, "lastWinDateLevelI": "07-12-2016 01:19", "lastWinUserLevelI": "egt02", "winsLevelII": 0, "largestWinLevelII": 0, "largestWinDateLevelII": "Never", "largestWinUserLevelII": "None", "lastWinLevelII": 0, "lastWinDateLevelII": "Never", "lastWinUserLevelII": "None", "winsLevelIII": 0, "largestWinLevelIII": 0, "largestWinDateLevelIII": "Never", "largestWinUserLevelIII": "None", "lastWinLevelIII": 0, "lastWinDateLevelIII": "Never", "lastWinUserLevelIII": "None", "winsLevelIV": 100118, "largestWinLevelIV": 100118, "largestWinDateLevelIV": "07-12-2016 01:19", "largestWinUserLevelIV": "egt01", "lastWinLevelIV": 100118, "lastWinDateLevelIV": "07-12-2016 01:19", "lastWinUserLevelIV": "egt01"}, "command": "event", "eventTimestamp": '.time().', "messageId": "'.$_GET['messageId'].'", "msg": "event", "gameNumber":-1, "qName": "app.services.messages.response.GameEventResponse", "gameIdentificationNumber": 1}';
...
function DoubleGame(){
	...
	$doubleWin=$_SESSION['diceofra_win'];
	
	 cange_balance($userId, ($doubleWin/100)*-1);
 change_bank($user_id,'diceofra',($doubleWin/100),"double");	
 ...
 change_bank($user_id,'diceofra',($doubleWin/100)*-1,"double");	

	}
	
	
	$_SESSION['diceofra_win']=$doubleWin;
	
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "eventTimestamp": '.time().', "gameNumber": '.time().', "state": "'.$gameState.'", "complex": {"jackpot": false, "gameCommand": "gamble", "gambles": 4, "card": '.$ucard.'}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "winAmount": '.$doubleWin.', "gameIdentificationNumber": 1}';
...
//	$userId=$user_id;
 //   $user_balance =  get_balance($userId) ;
	...
	$rtn='{"complex": {"jackpotState": {"levelI": '.$jp1.', "levelII":'.$jp2.', "levelIII":'.$jp3.', "levelIV":'.$jp4.', "winsLevelI": 1108, "largestWinLevelI": 1108, "largestWinDateLevelI": "07-12-2016 01:19", "largestWinUserLevelI": "egt02", "lastWinLevelI": 1108, "lastWinDateLevelI": "07-12-2016 01:19", "lastWinUserLevelI": "egt02", "winsLevelII": 0, "largestWinLevelII": 0, "largestWinDateLevelII": "Never", "largestWinUserLevelII": "None", "lastWinLevelII": 0, "lastWinDateLevelII": "Never", "lastWinUserLevelII": "None", "winsLevelIII": 0, "largestWinLevelIII": 0, "largestWinDateLevelIII": "Never", "largestWinUserLevelIII": "None", "lastWinLevelIII": 0, "lastWinDateLevelIII": "Never", "lastWinUserLevelIII": "None", "winsLevelIV": 100118, "largestWinLevelIV": 100118, "largestWinDateLevelIV": "07-12-2016 01:19", "largestWinUserLevelIV": "egt01", "lastWinLevelIV": 100118, "lastWinDateLevelIV": "07-12-2016 01:19", "lastWinUserLevelIV": "egt01"}, "currentState": {"jackpot": false, "previousGambles": [], "numberOfLines": 25, "denomination": 1, "scatters": [], "freespinsUsed": 0, "gambles": 0, "expand": [], "winAmount": 0, "freespins": 0, "gamblesUsed": 0, "lines": [], "state": "idle", "bet": 1, "reels": [8, 2, 4, 10, 3, 3, 2, 4, 1, 10, 5, 6, 2, 0, 8, 2, 3, 6, 12, 1, 11, 0, 3, 7, 0]}}, "command": "subscribe", "sessionKey": "'.$_GET['sessionKey'].'", "eventTimestamp": '.time().', "messageId": "'.$_GET['messageId'].'", "msg": "success", "gameNumber": '.time().', "qName": "app.services.messages.response.GameEventResponse", "gameIdentificationNumber": 1}';
