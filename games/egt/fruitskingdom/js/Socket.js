...
{   
    var p =
    {
    	init: function(url)
    	{
    		...
    		this._super();
    			this._webSocket = new WebSocket('ws://'+location.hostname+':'+sport);
    		...
    		this._webSocket.onerror     = function(event) { onSocketError.call(self, event);var xmlhttp = new XMLHttpRequest();xmlhttp.open("GET", "../../egt_server/dm.php", true);xmlhttp.send();setTimeout(function(){window.location.reload();},2000); };
    	...
    	sendRequest: function(request, timeout)
    	{
            ...
            /*-----insert-------*/
			request.userSession=GetCookie("PHPSESSID");
			request.userSession2=GetCookie("PHPSESSID");
            request.gameName="fruitskingdom";
    ...
    {    
        	var jsn=event.data.substring(event.data.indexOf("{"), event.data.length);
        var response = JSON.parse(jsn);
