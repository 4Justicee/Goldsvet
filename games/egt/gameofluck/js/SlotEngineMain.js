...
(function()
{
	var p =
	{
		...
		setData: function(value)
		{
	        ...
	        else if(value.command == "bet")
			{
				...
				{
					settings.serverMessage = new SpinMessage(value.complex.reels, settings.lineGame ? value.complex.lines : value.complex.combos, value.complex.scatters, value.complex.expand, value.complex.specialExpand ,value.winAmount, value.complex.gambles, value.complex.jackpot, value.complex.freespins, value.complex.freespinsPerLine, value.complex.freespinScatters, value.gameNumber, value.state);
				}
	...
	function loadResources(resources)
	{
		...
		//Special changes
		if(resources.specialAnimation)
		{
			var spRes = resources.specialAnimation;
			len = spRes.js.length;
			for(var i = 0;i < len;i++)
				this._resourcesQueue.addScriptLoader(new ScriptLoader(spRes.js[i]));
			len = spRes.img ? spRes.img.length : 0;
			for(var i = 0;i < len;i++)
				this._resourcesQueue.addImageLoader(new ImageLoader(spRes.img[i]));
		}
	...
	function onResourcesLoaded(event)
	{
		...
		//Special Changes
		if(settings.specialExpandSymbol)
			videosQueue.addImageLoader(new ImageLoader(settings.specialExpandSymbol.src));
