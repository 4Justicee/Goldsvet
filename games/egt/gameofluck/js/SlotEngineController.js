...
//			var debugTextView = new TextView("debugTextView", "DASDASD", 280, 200);
//			this._view.addChild(debugTextView);
//			debugTextView.setText(navigator.userAgent);
			...
			else if (currentState.state == BaseMessage.STATE_GAMBLE)
			{
				...
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines, [], currentState.expand,null, currentState.winAmount,
			...
			else if (currentState.state == BaseMessage.STATE_FREESPIN)
			{
				...
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines, currentState.scatters, currentState.expand,null, currentState.winAmount,
				...
				if (currentState.firstSpin && settings.restoreReels)
					this._freespinsSpinMessage = new SpinMessage(currentState.firstSpin.reels.slice(), currentState.firstSpin.lines ? currentState.firstSpin.lines.slice() : currentState.firstSpin.combos.slice(), currentState.firstSpin.scatters.slice(), [],null, 0, 0, false, 0, null, [], 0, "");
		...
		spinReceived: function()
		{
			...
			if (spinMessage.freeSpins && !this._freespinsSpinMessage && settings.restoreReels)
				this._freespinsSpinMessage = new SpinMessage(spinMessage.reels, spinMessage.lines, spinMessage.scatters, [],null, 0, 0, false, 0, null, [], 0, "");
	...
	function startVideos()
	{
		...
		//SpecialChanges
		this._videoViewController.addEventListener(VideoViewController.SCATTER_COMPLETE, onScatterComplete, this);
		//End
	...
	function stopVideos(redrawReels)
	{
		...
		//SpecialChanges
		this._videoViewController.removeEventListener(VideoViewController.SCATTER_COMPLETE, onScatterComplete, this);
		//End
	...
	function onPaytableButtonClick() { createPaytable.call(this); }
	...
	function onReelsStopped()
	{
		...
		{
			if (spinMessage.scatters.length && spinMessage.expand.length)
			{
				startWinVisualization.call(this);
				this._uiViewController.setUIState(UserInterfaceViewController.STATE_WIN, settings.autoplayGame);
				this._uiViewController.setWinMoney(spinMessage.scattersWinAmount);
			}
			startVideos.call(this);
		}
	...
	//SpecialChanges
	function onScatterComplete(event)
	{
		var settings = GameSettings.getInstance();
		var spinMessage = settings.serverMessage;
		this._scatterComplete = true;
		if(!this._uiViewController.getWinMoneyAnimating())
		{
			stopVideos.call(this);
			stopWinVisualization.call(this);
			spinMessage.scatters.length = 0;
			startVideos.call(this);
			this._uiViewController.setUIState(UserInterfaceViewController.STATE_FREESPINS_TRANSITION);
		}
	}
