	...
	var p =
	{
		init: function(isLinearWin)
		{
			...
			if (isLinearWin) {
				...
				var len = spinMessage.freeSpinsExpandLines.length;
				for (var i = 0; i < len; i++) {
					var cellsExp = spinMessage.freeSpinsExpandLines[i].cells;
					var cellsLen = cellsExp.length;
					for (var j = 0; j < cellsLen; j+=2)
						createVideo.call(this, cellsExp[j], cellsExp[j + 1], false);
				}
			}
