...
(function()
{
	var p = 
	{
		init: function(config, reelIndex)
		{
			...
			this._img = Loader.getLoader(settings.engineType+'/games/'+settings.gameType+'/images/frame.png').getData();
		},
		...
		getDistToRest: function()
		{
			if (this._state == ReelView.REEL_BOUNCING)
				return Math.abs(this._offset - (this._final_offset - this._bounce));
			return -1;
		},
		...
		{
			this._config.context.clearRect(0, 0, 1, 1);
