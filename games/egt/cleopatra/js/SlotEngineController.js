...
(function()
{
	var p =
	{
		init: function(currentState)
		{
			...
			this._freeSpinsExpandedSymbolsController;
			...
			else if (currentState.state == BaseMessage.STATE_GAMBLE)
			{
				...
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines, currentState.scatters, currentState.expand, currentState.winAmount,
														 currentState.gambleAttemptsLeft, currentState.jackpot, currentState.freeSpins, currentState.freeSpinsPerLine, currentState.freespinScatters,
														 currentState.freeSpinsExpandLines, currentState.freeSpinsExpandSymbol, currentState.freeSpinsExpandWinAmount,
														 currentState.gameNumber, currentState.state);
			...
			{
				settings.freeSpinsExpandSymbol = currentState.freeSpinsExpandSymbol;
				...
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines, currentState.scatters, currentState.expand, currentState.winAmount,
														 ...
														 currentState.freespinScatters, null, settings.freeSpinsExpandSymbol, -1,
														 currentState.gameNumber, currentState.state);
				...
				if (currentState.firstSpin && settings.restoreReels)
					this._freespinsSpinMessage = new SpinMessage(currentState.firstSpin.reels.slice(), currentState.firstSpin.lines ? currentState.firstSpin.lines.slice() : currentState.firstSpin.combos.slice(), currentState.firstSpin.scatters.slice(), currentState.firstSpin.expand.slice(), 0, 0, false, 0, null, [], null, -1, -1, 0, "");

				if (currentState.freespinsUsed == 0)
					startVideos.call(this);

				// Add frame to special expand symbol to reelsView images and customResultView
				if (currentState.freespinsUsed > 0) {
					settings.assembleExpandCards();
					this._reelsViewController.updateReelImages();

					if(this._customResultViewController)
						this._customResultViewController.updateReelImages();
				}
		...
		spinReceived: function()
		{
			...
			if (spinMessage.freeSpins && !this._freespinsSpinMessage && settings.restoreReels)
				this._freespinsSpinMessage = new SpinMessage(spinMessage.reels, spinMessage.lines, spinMessage.scatters, [], 0, 0, false, 0, null, [], null, -1, -1, 0, "");
	...
	function startVideos(isLinearWin)
	{
		if (this._videoViewController)
			return;

		this._videoViewController = new VideoViewController(isLinearWin);
		...
		if (this._winVisualizationViewController)
			this.addChildViewController(this._videoViewController, this._winVisualizationViewController);
		else if (!isLinearWin)
			this.addChildViewController(this._videoViewController, this._freeSpinsExpandedSymbolsController);
	...
	function onReelsStopped()
	{
		...
		{
			if (settings.freespinGame && spinMessage.freeSpinsExpandWinAmount > 0) {
				initializeFreeSpinsExpandedSymbols.call(this);
				return;
			}
	...
	function onExpandComplete()
	{
		...
		{
			if (spinMessage.freeSpins && !settings.isFreeSpinsExpandingBegin)
		...
		if (!settings.isFreeSpinsExpandingBegin)
	...
	function onWinMoneyAnimationComplete()
	{
		...
		if (this._stateMachineContext.getState() == StateMachineContext.STATE_WIN)
		{
			...
			if (this._winVisualizationViewController) {
				// this._winVisualizationViewController.stopCoinAnimation();
				this._winVisualizationViewController.allowCreditAnimationSound = false;
			}
			if (!spinMessage.freeSpins && this._winVisualizationViewController && !this._winVisualizationViewController.getCardSoundPlaying())
			{
				this._winVisualizationViewController.stopWinSound();
			}
			...
			if (settings.freespinGame && spinMessage.freeSpinsExpandWinAmount > 0) {
				if (!this._freeSpinsExpandedSymbolsController) {
					initializeFreeSpinsExpandedSymbols.call(this);
					return;
				} else if (spinMessage.freeSpins || settings.currFreespin == settings.totalFreespins || !this._lostConnection) {
					spinMessage.reels = spinMessage.freeSpinsExpandReels.reels.slice();
					spinMessage.lines = spinMessage.freeSpinsExpandReels.lines.slice();
					...
					settings.isFreeSpinsExpandingBegin = false;
					this._freeSpinsExpandedSymbolsController.destroyExpandedWins();

					// Destroy free spins expanded symbols
					destroyFreeSpinsExpandedSymbols.call(this);

					if (spinMessage.freeSpins)
						onEnterFreespinsScenarioComplete.call(this, null);
				}
			}
	...
	function onFSSpecialExpandAnimationComplete(event) {


		this._freespinAnimationViewController.playIntroOut();
	}
	function onFSRefreshScreen(event)
	{
		...
		{
			// if (this._winVisualizationViewController)
			// 	this._winVisualizationViewController.freespins = false;
			this._uiViewController.freespins(false);

			// Restore expand image to reels images
			settings.assembleExpandCards();
			// Restore reels view to its initial state
			this._reelsViewController.updateReelImages();
			this._reelsViewController.redrawReels();
			// Restore custom result panel view
	...
	function onFSIntroEnded(event)
	{
		var settings = GameSettings.getInstance();
		var message = settings.serverMessage;
		var specialExpand = message.freeSpinsExpandSymbol;
		var card = settings.cardsInfo[specialExpand].reelImageIndex;
	...
	function onSpecialExpandComplete(event) {
		var settings = GameSettings.getInstance();
		var spinMessage = settings.serverMessage;

		settings.isFreeSpinsExpandingBegin = true;

		startVideos.call(this, false);
		startWinVisualization.call(this);

		this._freeSpinsExpandedSymbolsController.destroyExpandedCards();
		this._uiViewController.setUIState(UserInterfaceView.STATE_WIN, false);
	}

	function initializeFreeSpinsExpandedSymbols() {
		var settings = GameSettings.getInstance();
		var spinMessage = settings.serverMessage;
		...
		if (spinMessage.freeSpinsExpandSymbol > -1) {
			this._freeSpinsExpandedSymbolsController = new FreeSpinsExpandedSymbolsController();
			this._freeSpinsExpandedSymbolsController.addEventListener(FreeSpinsExpandedSymbolsController.ON_EXPANDING_COMPLETED, onSpecialExpandComplete, this);

			this.addChildViewController(this._freeSpinsExpandedSymbolsController);

			this._freeSpinsExpandedSymbolsController.assembleExpanded();
			this._freeSpinsExpandedSymbolsController.start();
		}
	}

	function destroyFreeSpinsExpandedSymbols() {
		if (this._freeSpinsExpandedSymbolsController) {
			this._freeSpinsExpandedSymbolsController.removeEventListener(FreeSpinsExpandedSymbolsController.ON_EXPANDING_COMPLETED, onSpecialExpandComplete, this);
			this._freeSpinsExpandedSymbolsController.dispose();
			this._freeSpinsExpandedSymbolsController = null;
			this._reelsViewController.redrawReels();
		}
	}
