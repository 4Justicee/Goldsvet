...
$res1=mysql_fetch_assoc(mysql_query("SELECT g_id FROM game_settings WHERE g_name='olympus'"));
...
$_SESSION['olympus_fsc']=0;
$_SESSION['olympus_fsc2']=0;
	...
	$_SESSION['olympus_win']=$hisInfo['winAmount'];	
	}
	
	$userId=$user_id;
    $user_balance =get_balance($userId)*100 ;
	$user_balance-=$_SESSION['olympus_win'];
	
	return '{"sessionKey": "'.$_GET['sessionKey'].'", "eventTimestamp":'.time().', "qName": "app.services.messages.response.LoginResponse", "currency": "EUR", "multigame": true, "groups": ["all", "myGames", "fruit", "classic", "multiline", "extraline", "diceSlots", "keno", "roulette", "cards"], "showRtp": false, "autoplayLimit": 0, "playerName": "egt01", "languages": ["ro", "nl", "it", "bg", "en", "es", "ru", "fr", "go"], "complex": {"OGJSlot": [{"mlmJackpot": true, "recovery": "norecovery", "gameName": "Dragon Reels", "featured": false, "groups": [{"name": "all", "order": 16}, {"name": "fruit", "order": 5}, {"name": "classic", "order": 31}, {"name": "multiline", "order": 35}, {"name": "extraline", "order": 22}, {"name": "diceSlots", "order": 29}, {"name": "keno", "order": 16}, {"name": "roulette", "order": 16}, {"name": "cards", "order": 19}, {"name": "myGames", "order": 17}], "totalBet": 0, "gameIdentificationNumber": 1}]}, "command": "login", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.'}';	

	
	 //set_stat_game($userId, $user_balance, 0,0,"state_olympus");
	return $rtn;	
	}
	...
	if(!isset($_SESSION['olympus_fsc'])){
	$_SESSION['olympus_fsc']=0;	
	}
	
	if($_SESSION['olympus_fsc']<=0){
	 cange_balance($userId, $allbet*-1);
	 change_bank($user_id,'olympus',$allbet,"spin");
	}
	////////////////////////////////
    ...
    $ptb[0]=array(0,0,0,5,25,100);  // 10,
    $ptb[1]=array(0,0,0,5,25,100);  // j
    $ptb[2]=array(0,0,0,5,25,100);  // Q

    $ptb[3]=array(0,0,0,5,40,150); // K
    $ptb[4]=array(0,0,0,5,40,150);  //A

    $ptb[5]=array(0,0,5,30,100,750); // man
    $ptb[6]=array(0,0,5,30,100,750); // girl

    $ptb[7]=array(0,0,5,40,400,2000);
    $ptb[8]=array(0,0,10,100,1000,5000);
...
$isWin=false;
    ...
    if($reels[$ii][0]==$_SESSION['olympus_exp'] || $reels[$ii][1]==$_SESSION['olympus_exp'] || $reels[$ii][2]==$_SESSION['olympus_exp']){
	$ss[$ii]=1;	
	$wcnt++;
	}
	...
	$exPay=$ptb[$_SESSION['olympus_exp']][$wcnt]*$bet;
...
$isCalc=false;
 ...
 change_bank($user_id,'olympus',($totalWin/100)*-1,"spin");	
	...
	$gameState="freespin";
	if($_SESSION['olympus_fsc']<=0){
	$_SESSION['olympus_fsc']=11;
	$_SESSION['olympus_fsc2']=0;
	$_SESSION['olympus_exp']=rand(0,6);
	$fsc=10;
	}else{
	$_SESSION['olympus_fsc']+=10;
$fsc=10;	
	}
	}
	...
	if($_SESSION['olympus_fsc']<=0){
	$_SESSION['olympus_win']=$totalWin;
	$_SESSION['olympus_exp']=-1;
	}else{
	$_SESSION['olympus_win']+=$totalWin;
$totalWin=$_SESSION['olympus_win'];	
$gameState="idle";
  $user_balance =get_balance($userId)*100 ;
	}
	...
	if($_SESSION['olympus_fsc']>0){
	$_SESSION['olympus_fsc']--;	
	$_SESSION['olympus_fsc2']++;	
	...
	$_SESSION['olympus_j1']=0;
	$_SESSION['olympus_j2']=0;
	$_SESSION['olympus_j3']=0;
	$_SESSION['olympus_j4']=0;
	...
	$logstr.='winAmount='.$_SESSION['olympus_win'].'##';
	$logstr.='gameState='.$gameState.'##';
	$logstr.='ws='.$ws.'##';
	$logstr.='freespins='.$_SESSION['olympus_fsc'].'##';
	$logstr.='freespinsUsed='.$_SESSION['olympus_fsc2'].'##';
	$logstr.='scatters='.$scatterCells.'##';
	$logstr.='exp='.$_SESSION['olympus_exp'].'##';
	$logstr.='freespinScatters='.$scatterCells2.'##';
	
	$_SESSION['olympus_reelString']=$reelString;
	/*history*/
	//$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$totalWin.', "eventTimestamp": '.time().', "gameNumber": 1538645, "state": "'.$gameState.'", "complex": {"jackpot": false,"freeSpinsExpandSymbol":0,"freeSpinsExpandLines":[],"freeSpinsExpandWinAmount":0, "freespins": '.$_SESSION['olympus_fsc'].', "scatters": ['.$scatterCells.'], "expand": [], "gambles": 0, "gameCommand": "bet", "lines": ['.$ws.'], "reels": ['.$reelString.'], "freespinScatters": ['.$scatterCells2.']}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$totalWin.', "eventTimestamp": '.time().', "gameNumber": 1645733, "state": "'.$gameState.'", "complex": {"freeSpinsExpandSymbol": '.$_SESSION['olympus_exp'].', "jackpot": '.$isJackStr.', "scatters": ['.$scatterCells.'], "gambles": 5, "expand": [], "freespinScatters": ['.$scatterCells2.'], "freeSpinsExpandWinAmount": '.$fsWin.', "freeSpinsExpandLines": ['.$wsfs.'], "freespins": '.$fsc.', "lines": ['.$ws.'], "gameCommand": "bet", "reels": ['.$reelString.']}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';
...
$sets=get_game_settings('olympus');
...
set_game_settings ('olympus','g_bon1_1',$jacks_str);
...
global $demomode, $login,$userId,$user_id,$jp1,$jp2,$jp3,$jp4,$logstr;	
	...
	$_SESSION['olympus_j'.$jType]++;
	
	if($_SESSION['olympus_j'.$jType]>=3){
	$jackState="idle";	
	$sets=get_game_settings('olympus');
...
$jacks_str=implode("|",$jacks);
	...
	$_SESSION['olympus_win']=$winAmount;
	...
	/*history*/
	$logstr.='reelString='.$_SESSION['olympus_reelString'].'##';
	$logstr.='winAmount='.$_SESSION['olympus_win'].'##';
if($winAmount>0){
...
global $demomode, $login,$userId,$user_id,$logstr;	
	...
	$rtn='{"sessionKey": "'.$_GET['sessionKey'].'", "qName": "app.services.messages.response.GameEventResponse", "winAmount": '.$_SESSION['olympus_win'].', "eventTimestamp": '.time().', "gameNumber": 1561257, "state": "idle", "complex": {"gameCommand": "collect"}, "command": "bet", "messageId": "'.$_GET['messageId'].'", "msg": "success", "balance": '.$user_balance.', "gameIdentificationNumber": 1}';	
...
function DoubleGame(){
	...
	$doubleWin=$_SESSION['olympus_win'];
	
	 cange_balance($userId, ($doubleWin/100)*-1);
 change_bank($user_id,'olympus',($doubleWin/100),"double");	
 ...
 change_bank($user_id,'olympus',($doubleWin/100)*-1,"double");	

	}
	
	
	$_SESSION['olympus_win']=$doubleWin;
	/*history*/
	$logstr.='reelString='.$_SESSION['olympus_reelString'].'##';
	$logstr.='winAmount='.$_SESSION['olympus_win'].'##';
	$logstr.='gameState='.$gameState.'##';
...
function InitGame2(){
	...
	unset($_SESSION['olympus_j1']);
	unset($_SESSION['olympus_j2']);
	unset($_SESSION['olympus_j3']);
	unset($_SESSION['olympus_j4']);
	$hisInfoTmp=explode("##",$res2['str']);
	$hisInfo=array();
	for($i=0; $i<count($hisInfoTmp);$i++){
	$hisInfoTmp2=explode("=",$hisInfoTmp[$i]);	
	...
	$_SESSION['olympus_reelString']=$hisInfo['reelString'];
	...
	$_SESSION['olympus_fsc']=$hisInfo['freespins'];
	$_SESSION['olympus_fsc2']=$hisInfo['freespinsUsed'];
	}
	if(isset($hisInfo['freespins'])){
	$_SESSION['olympus_exp']=$hisInfo['freespins'];
	}
