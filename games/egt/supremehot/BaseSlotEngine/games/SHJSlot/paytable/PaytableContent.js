...
function setPaytableContent(paytableCoeficients, denominations, currDenomIndex, betPerLine, totalBet, jackpotMinBet, jackpotMaxBet, currencyType, currency)
{
	...
	for(var index in paytableCoeficients)
	{
		...
		for(var i = 0;i < len;i++)
		{
			...
			if(currency)
				div[0].innerHTML = Utils.formatNumber(betPerLine*paytableCoeficients[index].coef[i], 100, true) + " "+ "<span class='currency'>"+currencyType+"</span>";
			else
				div[0].innerHTML = Utils.formatNumber(betPerLine*paytableCoeficients[index].coef[i], denominations[currDenomIndex][0]);
		}
	}
