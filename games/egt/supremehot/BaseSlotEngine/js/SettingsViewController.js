...
(function()
{
	var p =
	{
		init: function(animated)
		{
			...
			settings.betValues=new Array(5,10,25,50,100);
			this._view = new SettingsView({denominations: denominations, currentDenom: settings.currDenominationIndex,
                            ...
                            isAutoplayGameDisabled: settings.autoplayLimit != null && settings.autoplayLimit[0] == 0,
							gameVersion: settings.gameVersion, paddingX: settings.paddingX, paddingY: settings.paddingY, scaleCoef: settings.scaleCoef});
		...
		updateSelectedBet: function() {
			this._view.setCurrBet(GameSettings.getInstance().currBetIndex);
		},
		
		updateSelectedDenomination: function() {
			this._view.setCurrDenom(GameSettings.getInstance().currDenominationIndex);
		},
	...
	function onDenomButtonClick(event)
	{
		...
		this.updateSelectedDenomination();
	...
	function onHelpButtonClick()
	{
		...
		params.hasAutoplayLimit = settings.autoplayLimit != null && settings.autoplayLimit.length > 0 && settings.autoplayLimit[0] != 0;
		...
		params.gameVersion = settings.gameVersion;
		params.isAutoplayDisabled = settings.autoplayLimit != null && settings.autoplayLimit[0] == 0;
