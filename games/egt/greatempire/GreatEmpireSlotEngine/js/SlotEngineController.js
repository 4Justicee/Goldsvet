...
(function()
{
	var p =
	{
		init: function(currentState)
		{
			...
			this._arrayWithSpecialWilds;
			this._selectedChoiseItem;
			this._holdReels = [];
			...
			else if (currentState.state == BaseMessage.STATE_GAMBLE)
			{
				...
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines,null, currentState.scatters, currentState.expand, currentState.winAmount,
														 currentState.gambleAttemptsLeft, currentState.jackpot, currentState.freeSpins, currentState.freespinScatters,0,null,currentState.gameNumber, currentState.holdReels,currentState.state);
			...
			else if (currentState.state == BaseMessage.STATE_FREESPIN || currentState.state == BaseMessage.STATE_CHOICE )
			{
				initialStateMachineContextState = StateMachineContext.STATE_WIN;
				settings.serverMessage = new SpinMessage(currentState.reels, currentState.lines,null,currentState.scatters, currentState.expand, currentState.winAmount,
														 currentState.gambleAttemptsLeft, currentState.jackpot, currentState.freespinsUsed == 0 ? currentState.freeSpins : 0,
														 currentState.freespinScatters,0,currentState.choiceObject,currentState.gameNumber,currentState.holdReels, currentState.state);
				...
				this._holdReels = currentState.holdReels ? currentState.holdReels.slice():[];
				for (var j = 0; j < settings.numReels; j++) {
					if ( this._holdReels.indexOf(j) == -1 ) {
						continue;
					};//
					for (var i = 0; i < settings.numReelCards + 2; i++) {
						spinMessage.reels[(settings.numReelCards + 2) * j + i] = settings.wildIndex;
					};//
				};//
				...
				if (currentState.firstSpin && settings.restoreReels)
					this._freespinsSpinMessage = new SpinMessage(currentState.firstSpin.reels.slice(), currentState.firstSpin.lines ? currentState.firstSpin.lines.slice() : currentState.firstSpin.combos.slice(),null, currentState.firstSpin.scatters.slice(), currentState.firstSpin.expand.slice(), 0, 0, false, 0, null, 0, null, 0,null, "");

				if (currentState.wildIndexes != undefined)
				{
					this._arrayWithSpecialWilds = currentState.wildIndexes;
					swapSpecialWilds.call(this);
				}

				if (currentState.state == BaseMessage.STATE_CHOICE && currentState.freespinsUsed == 0)
				{
					...
					createFreespinAnimation.call(this)
					this._freespinAnimationViewController.setParams({serverMessage:settings.serverMessage});
					this._freespinAnimationViewController.playIntro();
				}
			}
		...
		spinReceived: function()
		{
			...
			{
				this._uiViewController.updateTotalBet();
			}


			this._holdReels = spinMessage ? spinMessage.holdReels : null;
			...
			if (spinMessage.freeSpins && !this._freespinsSpinMessage && settings.restoreReels)
				this._freespinsSpinMessage = new SpinMessage(spinMessage.reels, spinMessage.lines, null ,spinMessage.scatters, [], 0, 0, false, 0, [], 0, null, 0,null, "");
			...
			{
				if (spinMessage.jackpot && spinMessage.state == BaseMessage.STATE_GAMBLE)
		...
		choiceReceived: function()
		{
			...
			if (message.state != BaseMessage.STATE_CHOICE)
			{
				...
				settings.totalFreespins += message.freeSpins;
				this._holdReels = message.holdReels;
				for (var j = 0; j < settings.numReels; j++) {
					if ( this._holdReels.indexOf(j) == -1 ) {
						continue;
					};//
					for (var i = 0; i < settings.numReelCards + 2; i++) {
						message.reels[(settings.numReelCards + 2) * j + i] = settings.wildIndex;
					};//
				};//
				this._reelsViewController.setReels(message.reels);

			}

			if (this._arrayWithSpecialWilds != undefined)
				swapSpecialWilds.call(this);

			if (message.choiceObject.figures != undefined)
			{
				this._arrayWithSpecialWilds = message.choiceObject.figures[this._selectedChoiseItem].wildIndexes; // holds the special wild symbols that need to be swap
				swapSpecialWilds.call(this);
			}
			this._freespinAnimationViewController.setParams({spinMessage:message});
			this._freespinAnimationViewController.choiceReceived();
		},
		swapSpecialWilds: function()
		{
			// var settings:GameSettings = GameSettings.getInstance();
			// for (var i:int = 0; i < _arrayWithSpecialWilds.length; i++)
			// {
			// 	var tmp:* = Resources.getInstance().reelBitmaps[_arrayWithSpecialWilds[i]];
			// 	Resources.getInstance().reelBitmaps[_arrayWithSpecialWilds[i]] = Resources.getInstance().reelBitmaps[_arrayWithSpecialWilds[i] +settings.numImages];
			// 	Resources.getInstance().reelBitmaps[_arrayWithSpecialWilds[i] + settings.numImages] = tmp;
			// 	var tmpVideo:* = GameSettings.getInstance().reelVideoURLs[_arrayWithSpecialWilds[i]];
			// 	GameSettings.getInstance().reelVideoURLs[_arrayWithSpecialWilds[i]] = GameSettings.getInstance().specialWildVideoURLs[_arrayWithSpecialWilds[i]];
			// 	GameSettings.getInstance().specialWildVideoURLs[_arrayWithSpecialWilds[i]] = tmpVideo;
			// }

			// _reelsViewController.redrawReels();
		},
		...
		customChoiceResultReceived: function()
		{
			this._stateMachineContext.choice();
		},
		sendSpin: function()
		{
			var settings = GameSettings.getInstance();
		...
		sendChoice: function()
		{
			this._selectedChoiseItem = this._freespinAnimationViewController.getChoice();
			this.dispatchEvent(new Event(SlotEngineMain.CHOICE, {gameCommand:BaseMessage.COMMAND_CHOICE, choice:this._freespinAnimationViewController.getChoice()}));
		},
		...
		sendCustomChoiceResult: function()
		{
			var settings = GameSettings.getInstance();
			if (settings.customResult)
			{
				var bet = this._customResultViewController.getResult();
				if (bet)
					bet.gameCommand = BaseMessage.COMMAND_SET_CHOICE_RESULT;
				this.dispatchEvent(new Event(SlotEngineMain.SET_CHOICE_RESULT, bet));
			}
			else
				throw "Trying to set custom result but \"customResult\" flag is set to false!";
		},
		...
		setWaitingForSpinResultVisualState: function()
		{
			...
			//hidePaytable();
			this._reelsViewController.start(this._holdReels);
		},
	...
	function createFreespinAnimation()
	{
		...
		this._freespinAnimationViewController.addEventListener(FreespinAnimationViewController.CHOICE_ENDED, onFSChoice, this);
	...
	function destroyFreespinAnimation()
	{
		...
		this._freespinAnimationViewController.removeEventListener(FreespinAnimationViewController.CHOICE_ENDED, onFSChoice, this);
	...
	function checkForFreespinEnd()
	{
		...
		if (settings.currFreespin == settings.totalFreespins)
		{
			...
			if (spinMessage.state != BaseMessage.STATE_CHOICE)
			{
	...
	function setTotalsInfo()
	{
		...
		{
			this._uiViewController.updateTotalBet();
			this._uiViewController.updateTotalWin();
		}
	}
	...
	function onAutoplayStopClick()
	{
		...
		if(message.state != BaseMessage.STATE_FREESPIN && message.state != BaseMessage.STATE_CHOICE && !message.jackpot)
	...
	function onPaytableButtonClick()
	{
		...
		if((GameType == "TDJSlot") || (GameType=="TWWJSlot") || (GameType == "ABJSlot") || (GameType == "BOMJSlot"))
			return;
	...
	function onReelsStopped()
	{
		...
		if(SpinAnalyzer.getNumWins(spinMessage) > 0|| spinMessage.freeSpins)
		{
	...
	function onEnterFreespinsScenarioComplete(event)
	{
		...
		{
			if (spinMessage.choices == 0)
					settings.totalFreespins += spinMessage.freeSpins;

			settings.totalFreespins += spinMessage.freeSpins;
			this._uiViewController.setUIState(UserInterfaceViewController.STATE_WAITING_FOR_FREESPINS_ANIMATION);
			this._freespinAnimationViewController.setParams({spinMessage:spinMessage});
			this._freespinAnimationViewController.playRetrigger();
			if (spinMessage.state == BaseMessage.STATE_CHOICE)
				this._freespinAnimationViewController.setPlayChoiceAfterRetrigger(true);
		}
	}
	...
	function onFSRefreshScreen(event)
	{
		...
		else
		{
			...
			if (this._arrayWithSpecialWilds != null)
			{
				swapSpecialWilds.call(this);
				this._arrayWithSpecialWilds = null;
			}
	...
	function onViewAddedToStage()
	{
		...
		if (this._currentState.state == BaseMessage.STATE_FREESPIN ||(this._currentState.state == BaseMessage.STATE_CHOICE && this._currentState.freespinsUsed != 0))
		{
			settings.freespinGame = true;
			...
			this._uiViewController.freespins(true);
			this._freespinsStartPressed = true;
			if (this._currentState.freeSpins)
			{
			...
			{
				createFreespinsAnimation.call(this);
				this._freespinAnimationViewController.setParams({spinMessage:spinMessage});
				this._freespinAnimationViewController.playIntro();
			}
		}
