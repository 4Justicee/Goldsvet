...
(function()
{
	var p =
	{
		init: function(config)
		{
			...
			this.setSize(1280*(1/config.scaleCoef), 720 + config.paddingY*2);
			
			this._scrollView = new ScrollView("paytableScrollView", config.paddingX, config.paddingY, true);
			this._scrollView.setSize(1280, 720 - config.paddingY*2);
			this._scrollView.setScale(1/config.scaleCoef, 1/config.scaleCoef);
		...
		show: function()
		{
			...
			var self = this;
			this._showHideTween = TweenMax.to(this._position, 0.2, {x: -this._config.paddingX, ease: Linear.easeNone, onComplete: function()
			{
