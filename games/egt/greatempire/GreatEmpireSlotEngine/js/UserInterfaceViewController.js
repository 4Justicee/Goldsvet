		...
		init: function()
		{
			...
			this._view = new UserInterfaceView({diamondPlay:settings.diamondPlay, currency:settings.currency, currencyType:settings.currencyType,
										   ...
										   betTitle:settings.localize("TOTAL BET"), winTitle:settings.localize("WIN"), freespinText:settings.localize("FREE SPIN"),
										  	ofText: settings.localize("OF"), totalBetLabel:settings.localize("totalBetLabel"), totalWinLabel:settings.localize("totalWinLabel"),
			   	lineGame:settings.lineGame, fixedLinesCount:settings.fixedLinesCount, jackpotAllowed: settings.jackpotAllowed,
			   	sendTotalsInfo: settings.sendTotalsInfo
			});
			...
			this._view.homeButton.addEventListener(MouseEvent.CLICK, onUIButtonClick, this);
...
var selff = this;
			$(document).keyup(function(event)
			{
				...
				{
					if(selff._view.spinButton.enabled())
					{
						selff._view.spinButton.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
					}
					else if(selff._view.collectButton.enabled())
					{
						selff._view.collectButton.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
					}
					else if(selff._view.stopButton.enabled())
					{
						selff._view.stopButton.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
					}
					else if(self._view.stopAutoplayButton.enabled())
					{
						selff._view.stopAutoplayButton.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
					}
				}
				else if(event.keyCode == 37 || event.keyCode == 39) // Left or Right
				{
					if(selff._view.gambleButton.enabled())
					{
						selff._view.gambleButton.dispatchEvent(new MouseEvent(MouseEvent.CLICK));
					}
				}
			});
			/*------------------*/	
		...
		updateTotalWin: function()
		{
			var settings = GameSettings.getInstance();

			this._view.updateTotalWin(settings.totalWin);
		},
		...
		updateTotalBet: function()
		{
			...
			this._view.updateTotalBet(settings.totalBet);
		},
