...
View("SlotEngineMain",
{
	...
	CHOICE: 			"choice", 				// sent when requesting a choice
	SET_RESULT:			"setResult",			// sent when a custom result should be set
	SET_CHOICE_RESULT:	"setChoice",			// sent when a custom result should be set
...
(function()
{
	var p =
	{
		init: function()
		{
			...
			this.events = [SlotEngineMain.SPIN, SlotEngineMain.COLLECT, SlotEngineMain.GAMBLE, SlotEngineMain.JACKPOT,SlotEngineMain.CHOICE, SlotEngineMain.SET_RESULT,SlotEngineMain.SET_CHOICE_RESULT, SlotEngineMain.HIDE_GAME];
		},
		...
		if (!isNaN(value.totalBet))
	        ...
	        else if(value.command == "bet")
			{
				...
				{
					settings.serverMessage = new SpinMessage(value.complex.reels, settings.lineGame ? value.complex.lines : value.complex.combos, value.complex.additional,value.complex.scatters, value.complex.expand, value.winAmount, value.complex.gambles, value.complex.jackpot, value.complex.freespins, value.complex.freespinScatters,value.choices,value.choiceObject, value.gameNumber,value.complex.holdReels, value.state);
				}
				...
				else if(value.complex.gameCommand == BaseMessage.COMMAND_CHOICE)
				{
					settings.serverMessage.state = value.state;
					settings.serverMessage.choiceObject = value.complex;
					settings.serverMessage.holdReels = value.complex.holdReels;
					settings.serverMessage.freeSpins = value.complex.freespins;
				}
	...
	function onResourcesLoaded(event)
	{
		...
		this._slotEngineController = new SlotEngineController(this._currentState);
