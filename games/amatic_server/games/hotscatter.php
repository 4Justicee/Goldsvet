...
$action="spin";
	...
	$rtn="#00101010";
...
$rtnReels.=strlen($rHex).$rHex;		
	...
	$_SESSION['dcard_hotscatter']=rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5)."";
	
	
	$rtn="1000$loginId".$rtnBalance."10".$rtnReels."000a10101010101010101010100b1010101010101010101010".$_SESSION['dcard_hotscatter']."f15fffff15fffff15fffff15fffff";
	 set_stat_game($userId, $user_balance, 0,0,"state_hotscatter");
	return $rtn;	
	}
...
if(!isset($_SESSION['dcard_hotscatter'])){
$_SESSION['dcard_hotscatter']=rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5).rand(0,5)."0";
	...
	///////////////////////////////
	 ...
	 change_bank($user_id,'hotscatter',$allbet,"spin");
...
if($winall>0){
 ...
 change_bank($user_id,'hotscatter',$winall*-1,"spin");	
...
$rtnReels.=strlen($rHex).$rHex;		
	...
	$_SESSION['hotscatter_freeGamesWin']=$winall;
	...
	$_SESSION['hotscatter_freeGames']=15;
	
	$_SESSION['hotscatter_freeGamesBank']=$gset['sum'];
...
$gameState="05";	
	...
	$rtn="1".$gameState."0".$loginId.$rtnBalance.strlen($winall_h).$winall_h.$rtnReels."".$bet_h.$ln_h.$freeInfo.$winstr.$_SESSION['dcard_hotscatter']."".$respinMap."#".count($r[3]);
	...
	$_SESSION['finish_answer']=strlen($winall_h).$winall_h.$rtnReels."".$bet_h.$ln_h."10101010101010101010100b".$winstr.$_SESSION['dcard_hotscatter'].$respinMap;
	
	set_stat_game($userId, $user_balance, $allbet,$winall,"spin_hotscatter");
	return $rtn;		
...
//////////////////
	...
	/////////////////////////////////////game_setting
		...
		$casbank = $_SESSION['hotscatter_freeGamesBank']; 
...
if($winall>0){
 ...
 cange_balance($userId, $winall);
 change_bank($user_id,'hotscatter',$winall*-1,"spin");	
$_SESSION['hotscatter_freeGamesBank']-=	$winall;
...
$rtnReels.=strlen($rHex).$rHex;		
	...
	$_SESSION['hotscatter_freeGamesWin']+=$winall;
	//$winall_r=$_SESSION['hotscatter_freeGamesWin'];
	...
	if($_SESSION['hotscatter_freeGames']<=0){
...
$ww="19";	
	...
	$freeRH=dechex($_SESSION['hotscatter_freeGames']);
	$freeR=dechex(strlen($freeRH)).$freeRH;
	 $freeInfo="1f".$freeR.$ww."131010".$rtnReels."0b";	
	/*-------------------------------*/
	$winall2=$_SESSION['hotscatter_freeGamesWin']*100;
	...
	$dc=$_SESSION['dcard_hotscatter'];
	$userId=$user_id;
 $_SESSION['dcard_hotscatter']=$dc;
	...
	$casbank=get_bank($user_id,'hotscatter',"double");	
	$winall=$_SESSION['double_win'];
	$dbet=$winall;
	
	if($winall>0){
	 cange_balance($userId, $winall*-1);
	 change_bank($user_id,'hotscatter',$winall,"double");	
...
$winall=sprintf( "%01.2f", $winall );
	...
	$doubleCards=$_SESSION['dcard_hotscatter'];
...
//bubi->chervy->tref->piki->
	...
	if($winall>0){
	 cange_balance($userId, $winall);
	 change_bank($user_id,'hotscatter',$winall*-1,"double");	
	...
	$_SESSION['dcard_hotscatter']=$doubleCards;
	$_SESSION['double_win']=$winall;
	
	
	set_stat_game($userId, $user_balance, $dbet,$winall,"double_hotscatter"); 
...
$winall=$winall/2;
	...
	$_SESSION['double_win']=$winall;
	set_stat_game($userId, $user_balance, $winall,$winall,"double_half_hotscatter");
	return $rtn;	
	}
