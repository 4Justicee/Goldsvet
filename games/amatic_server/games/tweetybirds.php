...
$action="spin";
	...
	$rtn="";
	return $rtn;   
...
$rtnReels.=strlen($rHex).$rHex;		
	...
	$rtn="1000$loginId".$rtnBalance."10".$rtnReels."00281010101010101010101010291010101010101010101010101010101010101010101010101010101010101010101010101010101010".$_SESSION['dcard_tweetybirds']."f15fffff15fffff15fffff15fffff";
...
$rtnReels.=strlen($rHex).$rHex;		
	...
	$_SESSION['tweetybirds_freeGamesT']=5;
...
$gameState="05";	
	...
	$rtn="1".$gameState."0".$loginId.$rtnBalance.strlen($winall_h).$winall_h.$rtnReels."".$bet_h.$ln_h.$freeInfo.$winstr.$_SESSION['dcard_tweetybirds']."".$respinMap."#".count($r[3]);
	...
	$_SESSION['finish_answer']=strlen($winall_h).$winall_h.$rtnReels."".$bet_h.$ln_h."101010101010101010101029".$winstr.$_SESSION['dcard_tweetybirds']."f15fffff15fffff15fffff15fffff";
...
$ww="1f";	
	...
	$freeR=dechex(strlen($freeRH)).$freeRH;
	 $freeInfo="1".$_SESSION['tweetybirds_freeGamesT'].$freeR.$ww."111010".$rtnReels."29";	
	/*-------------------------------*/
	$winall2=($_SESSION['tweetybirds_freeGamesWin']*100);
	$str_winall=str_replace(".","",$winall2."");
	
	if($map[8]==10 || $map[9]==10 || $map[10]==10 || $map[11]==10){
	$_SESSION['tweetybirds_freeGamesT']++;	
	$_SESSION['tweetybirds_freeGames']++;
	}
	...
	$rtn="1".$gameState."0".$loginId.$rtnBalance.strlen($winall_h).$winall_h.$rtnReels."".$bet_h.$ln_h.$freeInfo.$winstr.$_SESSION['dcard_tweetybirds']."".$respinMap."#";
